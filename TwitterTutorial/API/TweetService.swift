...
struct TweetService {
    ...
    func uploadTweet(caption: String, type: UploadTweetConfiguration, competion: @escaping(Error?, DatabaseReference) -> Void) {
        ...
        case .reply(let tweet):
            ...
            REF_TWEET_REPLIES.child(tweet.tweetID).childByAutoId().updateChildValues(values) { (err, ref) in
                ...
                REF_USER_REPLIES.child(uid).updateChildValues([tweet.tweetID: replyKey], withCompletionBlock: competion)
            }
        }
    }
    ...
    func fetchTweets(forUser user: User, completion: @escaping([Tweet]) -> Void) {
        ...
        REF_USER_TWEETS.child(user.uid).observe(.childAdded) { snapshot in
            ...
            self.fetchTweet(withTweetI: tweetID) { tweet in
                tweets.append(tweet)
                completion(tweets)
            }
        }
    ...
    func fetchTweet(withTweetI tweetID: String, completion: @escaping(Tweet) -> Void ) {
    ...
    func fetchLikes(forUser user: User, completion: @escaping([Tweet]) -> Void) {
        ...
        REF_USER_LIKES.child(user.uid).observe(.childAdded) { snapshot  in
            let tweetID = snapshot.key
            self.fetchTweet(withTweetI: tweetID) { likeTweet in
                var tweet = likeTweet
                tweet.didLike = true
                tweets.append(tweet)
                completion(tweets)
